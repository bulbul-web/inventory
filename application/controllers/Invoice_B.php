...
//        $cc = $lastid->max_id+1; //last_id
        ...
        $lastid = $this->db->query('SELECT voucher_id FROM tbl_invoice ORDER BY voucher_id DESC LIMIT 1')->row();
...
//        exit();
    ...
    public function update_invoice(){
        ...
        for($x=0;$x<count($data1);$x++){
             ...
             $pro_id = $data1[$x]['pro_id'];
             $result = $this->db->query("SELECT * FROM tbl_invoice WHERE voucher_id = '$voucher_id' AND product_id = $pro_id")->num_rows();
             if( $result > 0){
