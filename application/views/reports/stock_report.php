                    ...
                    if(isset($_POST['submit'])){
                        ...
                        $stock_out_result = $this->db->query
                                (
                                     ...
                                     "SELECT i.product_id, sum(i.quantity) as stock_out, pi.pack_size, ps.pack_size, pi.product_name FROM tbl_invoice i LEFT JOIN tbl_product_info pi ON pi.product_id = i.product_id LEFT JOIN tbl_pack_size ps ON pi.pack_size = ps.id WHERE i.product_id = $product_name AND NOT (i.delete_status <=> 'deleted') AND NOT (i.status <=> '0')"
                                )->row();
